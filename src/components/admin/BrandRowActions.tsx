...
import { deleteBrand, toggleBrandActive } from '@/lib/brands/actions'
...
export function BrandRowActions({ id, name, isActive }: Props) {
  ...
  function handleToggle() {
    ...
    startTransition(async () => {
      const result = await toggleBrandActive(id, !isActive)
  ...
  function handleDelete() {
    if (
      !confirm(
        `Xóa hãng "${name}"?\n\nLưu ý: nếu hãng đã có sản phẩm sẽ không xóa được.`
      )
    )
      return
