...
export function ProductRowActions({ id, name, isActive }: Props) {
  ...
  function handleDelete() {
    if (!confirm(`Xóa sản phẩm "${name}"?\n\nLưu ý: nếu sản phẩm đã có trong đơn hàng cũ sẽ không xóa được — hãy "Tắt" thay vì xóa.`))
      return
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      {error && (
        <span className="ml-2 text-xs text-status-out max-w-[200px] truncate" title={error}>
          {error}
        </span>
      )}
    </div>
